...
def get_ribbon(dimensions: str) -> int:
    ...
    ribbon: int = wrapping_ribbon + bow
    logger.debug(f'wrapping_ribbon={wrapping_ribbon}, bow={bow} -> ribbon={ribbon}')
    return ribbon


def day_02_part_2() -> int:
    list_of_dimensions: list[str] = read_puzzle_input('02-input.txt')
    total: int = sum([get_ribbon(dimensions) for dimensions in list_of_dimensions])
    return total


if __name__ == '__main__':
    logger.info(day_02_part_2())
